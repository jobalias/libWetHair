...
StrandForce::StrandForce(TwoDScene<3>* scene,
  ...
  m_lambda.resize(numConstraintNonViscous());
  m_lambda.setZero();
  m_lambda_v.resize(numConstraintViscous());
  m_lambda_v.setZero();
}
...
void StrandForce::updateStartDoFs(const VecX& x_startOfStep) {
  const VecX& currentStrandDoFs = x_startOfStep.segment(
...
void StrandForce::recomputeGlobal() {
  ...
  m_hess.setFromTriplets(m_strandHessianUpdate.begin(),
                         m_strandHessianUpdate.end());
...
void StrandForce::computeIntegrationVars(
  ...
  StretchingForce<NonViscous>::accumulateIntegrationVars(
      ...
      global_start_dof, *this, combinedLambda, J, tildeK, stiffness, Phi,
      poffset);
  poffset += getNumEdges();
  ...
  TwistingForce<NonViscous>::accumulateIntegrationVars(
      ...
      combinedLambda, J, tildeK, stiffness, Phi, poffset);
...
int StrandForce::numJ() {
  ...
  if (m_strandParams
          ->m_accumulateWithViscous) {  // double the active forces for viscous
    if (!m_strandParams->m_accumulateViscousOnlyForBendingModes) {
#ifdef STRETCH
      numJ += (getNumEdges() * 6);  // Stretch
#endif
...
int StrandForce::numJv() {  // viscous now treated via positions
  return 0;
}
...
int StrandForce::numConstraintVel() {  // now treated via pos
  return 0;
}
...
void StrandForce::getAffectedVars(int colidx, std::unordered_set<int>& vars) {
  ...
  for (int v = 0; v < getNumVertices(); ++v) {
    if (m_verts[v] == ip) {
      ...
      if (v != 0) {
        for (int r = 0; r < 4; ++r)
          vars.insert(m_scene->getDof(m_verts[v - 1]) + r);
      }
  ...
  for (int v = 0; v < getNumVertices(); ++v) {
    if (m_verts[v] == ip) {
      if (m_scene->isTip(ip)) {
        for (int r = 0; r < 3; ++r)
          vars.push_back(
              std::pair<int, int>(4 * v + r, m_scene->getDof(ip) + r));
      } else {
        for (int r = 0; r < 4; ++r)
          vars.push_back(
              std::pair<int, int>(4 * v + r, m_scene->getDof(ip) + r));
      ...
      if (v != 0) {
        for (int r = 0; r < 4; ++r)
          vars.push_back(std::pair<int, int>(
              4 * (v - 1) + r, m_scene->getDof(m_verts[v - 1]) + r));
      }
      if (v != getNumVertices() - 1) {
        for (int r = 0; r < 3; ++r)
          vars.push_back(std::pair<int, int>(
