...
TwoDScene<DIM>::TwoDScene(const TwoDScene& otherscene, const bool& isMassSpring)
  ...
  for (std::vector<Force*>::size_type i = 0; i < m_forces.size(); ++i)
    m_forces[i] = otherscene.m_forces[i]->createNewCopy();
}
...
void TwoDScene<DIM>::initializeCenters() {
  if (m_x.size() >= DIM) {
    ...
    m_collision_keep_proximity =
        m_edge_radii.sum() / (scalar)(m_edge_radii.size());

    m_search_radius =
        m_collision_keep_proximity * m_parameters.max_limited_eta_prop * 2.0;
  }
}
...
template <int DIM>
scalar TwoDScene<DIM>::getStiffnessPlanar(const scalar& radius,
                                          const scalar& d0,
                                          const scalar& A_target,
                                          const scalar& pressure_weight) const {
  return getPolygonalCohesion()->getStiffnessPlanar(radius, d0, A_target,
                                                    pressure_weight);
}
...
template <int DIM>
scalar TwoDScene<DIM>::getStiffness(const scalar& radius, const scalar& d0,
                                    const scalar& A_target,
                                    const scalar& pressure_weight) const {
  return getPolygonalCohesion()->getStiffness(radius, d0, A_target,
                                              pressure_weight);
}
...
void TwoDScene<DIM>::initializeLiquids() {
  ...
  if (m_parameters.mass_update_mode != MUM_NONE ||
      m_parameters.regularizer_shell > 0.0)
    updateHairMass();
...
void TwoDScene<DIM>::advectHairFlows(const scalar& dt) {
  ...
  threadutils::thread_pool::ParallelFor(
      0, nflows, [&](int i) { m_flows[i]->advance(m_x, dt); });
}
...
template <int DIM>
void TwoDScene<DIM>::addForceHairFlows(const VectorXs& accel,
                                       const scalar& dt) {
  if (!m_parameters.individual_transfer && m_polygonal_cohesion)
...
template <int DIM>
void TwoDScene<DIM>::updateHairFlowsHeight(const VectorXs& accel,
                                           const scalar& dt) {
  const int nflows = m_flows.size();
  threadutils::thread_pool::ParallelFor(
      0, nflows, [&](int i) { m_flows[i]->preUpdateHairFlowHeight(dt); });
...
template <int DIM>
void TwoDScene<DIM>::uniformlyIncreaseHairLiquid(const scalar& dh,
                                                 int group_idx) {
  int np = getNumParticles();
  for (int i = 0; i < np; ++i) {
    if (m_script_group[i] == group_idx) {
      ...
      m_flows[hidx]->getEta()(local_idx) =
          std::max(0.0, m_flows[hidx]->getEta()(local_idx) + dh);
    }
  }
}
...
void TwoDScene<DIM>::globalVolumeAdjust() {
  ...
  scalar new_vol_hair_lim = m_volume_hair_old + m_volume_particle_old -
                            m_volume_particle + m_volume_particle_inserted -
                            m_volume_particle_removed + m_volume_reservoir_old -
                            m_volume_reservoir;

  std::cout << "[vol statistics: old hair: " << m_volume_hair_old
            << " new hair: " << m_volume_hair
            << " old particle: " << m_volume_particle_old
            << " new particle: " << m_volume_particle
            << " inserted particle: " << m_volume_particle_inserted
            << " removed particle: " << m_volume_particle_removed
            << " old reservoir: " << m_volume_reservoir_old
            << " new reservoir: " << m_volume_reservoir << "]" << std::endl;

  if (m_volume_hair > 0.0 && m_volume_hair > new_vol_hair_lim) {
    scalar prop = new_vol_hair_lim / m_volume_hair;
    std::cout << "[volume adj: vol: " << m_volume_hair
              << " limit: " << new_vol_hair_lim << " prop: " << prop << "]"
              << std::endl;
...
void TwoDScene<2>::interpolateMass() {
  ...
  threadutils::thread_pool::ParallelFor(0, np, [&](int i) {
    ...
    m_interpolated_m.segment<2>(i * 2) =
        m_rest_m.segment<2>(i * 2) * clamped_particle_weight +
        m_m.segment<2>(i * 2) * (1.0 - clamped_particle_weight);
  });
}
...
void TwoDScene<3>::interpolateMass() {
  ...
  threadutils::thread_pool::ParallelFor(0, np, [&](int i) {
    ...
    if (m_massSpringSim || isTip(i)) {
      m_interpolated_m.segment<3>(getDof(i)) =
          m_rest_m.segment<3>(getDof(i)) * clamped_particle_weight +
          m_m.segment<3>(getDof(i)) * (1.0 - clamped_particle_weight);
    } else {
      m_interpolated_m.segment<4>(getDof(i)) =
          m_rest_m.segment<4>(getDof(i)) * clamped_particle_weight +
          m_m.segment<4>(getDof(i)) * (1.0 - clamped_particle_weight);
    }
  });
}
...
template <int DIM>
const Eigen::Quaternion<scalar>& TwoDScene<DIM>::getScriptedRotation(
    int sg_idx) const {
  return m_scripted_rotation[sg_idx];
}

template <int DIM>
void TwoDScene<DIM>::initializeScriptedGroup(
  ...
  for (const Script& s : scripts) {
    if (s.target == Script::ROOT && s.start < 0.0) {
      switch (s.type) {
        case Script::ROTATE: {
          m_scripted_rotation[std::max(0, s.index)] =
              Eigen::Quaternion<scalar>(Eigen::AngleAxis<scalar>(
                  s.v(3), Vector3s(s.v(0), s.v(1), s.v(2))));
          break;
        }
        case Script::TRANSLATE: {
          m_scripted_translate[std::max(0, s.index)] =
              Vector3s(s.v(0), s.v(1), s.v(2));
          break;
        }
        case Script::SCALE: {
...
void TwoDScene<DIM>::applyScript(const scalar& dt) {
  ...
  if (DIM == 3 && !m_massSpringSim) {
    ...
    for (int i = 0; i < nstrand; ++i) {
      if (!m_strandEquilibriumParameters[i] ||
          !m_strandEquilibriumParameters[i]->m_valid ||
...
template <int DIM>
void TwoDScene<DIM>::resizeSystem(int num_particles, int num_edges,
                                  int num_strands) {
  if (!m_massSpringSim) {
...
template <int DIM>
void TwoDScene<DIM>::resizeSystemDER(int num_particles, int num_edges,
                                     int num_strands) {
  assert(num_particles >= 0);

  int numDofs = (4 * num_particles) -
                num_strands;  // [ x y z t ] for all strand vertices except tip,
...
void TwoDScene<DIM>::setMass(int particle, const scalar& mass) {
  ...
  if (m_massSpringSim) {
    ...
    std::cerr << "mass computed via strand parameters in "
                 "TwoDScene<3>::computeMassesAndRadiiFromStrands() "
              << std::endl;
    exit(1);
  }
}
...
template <int DIM>
void TwoDScene<DIM>::setEdge(int idx, const std::pair<int, int>& edge,
                             scalar radius) {
  m_edges[idx] = edge;
...
template <int DIM>
void TwoDScene<DIM>::insertStrandEquilibriumParameters(
...
scalar TwoDScene<DIM>::computeKineticEnergy() const {
  return computeHairParticleKineticEnergy() +
         computeLiquidParticleKineticEnergy();
}
...
template <int DIM>
StrandEquilibriumParameters* TwoDScene<DIM>::getStrandEquilibriumParameters(
    const int index) {
  assert(0 <= index);
...
template <int DIM>
void TwoDScene<DIM>::accumulateExternalGradU(VectorXs& F, const VectorXs& dx,
                                             const VectorXs& dv) {
  ...
  if (dx.size() == 0)
    for (std::vector<Force*>::size_type i = 0; i < m_external_forces.size();
         ++i) {
    ...
    for (std::vector<Force*>::size_type i = 0; i < m_external_forces.size();
         ++i) {
      m_external_forces[i]->addGradEToTotal(m_x + dx, m_v + dv,
                                            m_interpolated_m, F);
    }
}

template <int DIM>
void TwoDScene<DIM>::accumulateGradU(VectorXs& F, const VectorXs& dx,
                                     const VectorXs& dv) {
  assert(F.size() == m_x.size());
  assert(dx.size() == dv.size());
  assert(dx.size() == 0 || dx.size() == F.size());
...
template <int DIM>
void TwoDScene<DIM>::accumulateddUdxdx(TripletXs& A, const VectorXs& dx,
                                       const VectorXs& dv) {
  if (dx.size() == 0)
    for (std::vector<Force*>::size_type i = 0; i < m_forces.size(); ++i)
      m_forces[i]->addHessXToTotal(m_x, m_v, m_interpolated_m, A);
  else
    for (std::vector<Force*>::size_type i = 0; i < m_forces.size(); ++i)
      m_forces[i]->addHessXToTotal(m_x + dx, m_v + dv, m_interpolated_m, A);
}

template <int DIM>
void TwoDScene<DIM>::accumulateddUdxdv(TripletXs& A, const VectorXs& dx,
                                       const VectorXs& dv) {
  if (dx.size() == 0)
    for (std::vector<Force*>::size_type i = 0; i < m_forces.size(); ++i)
      m_forces[i]->addHessVToTotal(m_x, m_v, m_interpolated_m, A);
  else
    for (std::vector<Force*>::size_type i = 0; i < m_forces.size(); ++i)
      m_forces[i]->addHessVToTotal(m_x + dx, m_v + dv, m_interpolated_m, A);
}

template <int DIM>
void TwoDScene<DIM>::accumulateExternalddUdxdx(TripletXs& A, const VectorXs& dx,
                                               const VectorXs& dv) {
  if (dx.size() == 0)
    for (std::vector<Force*>::size_type i = 0; i < m_external_forces.size();
         ++i)
      m_external_forces[i]->addHessXToTotal(m_x, m_v, m_interpolated_m, A);
  else
    for (std::vector<Force*>::size_type i = 0; i < m_external_forces.size();
         ++i)
      m_external_forces[i]->addHessXToTotal(m_x + dx, m_v + dv,
                                            m_interpolated_m, A);
}

template <int DIM>
void TwoDScene<DIM>::accumulateExternalddUdxdv(TripletXs& A, const VectorXs& dx,
                                               const VectorXs& dv) {
  if (dx.size() == 0)
    for (std::vector<Force*>::size_type i = 0; i < m_external_forces.size();
         ++i)
      m_external_forces[i]->addHessVToTotal(m_x, m_v, m_interpolated_m, A);
  else
    for (std::vector<Force*>::size_type i = 0; i < m_external_forces.size();
         ++i)
      m_external_forces[i]->addHessVToTotal(m_x + dx, m_v + dv,
                                            m_interpolated_m, A);
}
...
void TwoDScene<DIM>::checkConsistency() {
  ...
  if (m_massSpringSim)
    assert(m_x.size() == (int)(DIM * m_fixed.size()));
  else
    assert(m_x.size() == (int)(4 * m_fixed.size() - getNumFlows()));
...
void TwoDScene<DIM>::addVolSummary() {
  if (m_parameters.volume_summary) {
    ...
    std::cout << "[Volume Summary:\t" << sum_liq << " + " << sum_res << " + "
              << sum_free << " = " << (sum_liq + sum_res + sum_free) << "\t]"
              << std::endl;
  }
}

template <int DIM>
void TwoDScene<DIM>::preCompute(const VectorXs& dx, const VectorXs& dv,
                                const scalar& dt) {
  assert(dx.size() == dv.size());
  ...
  if (dx.size() == 0) {
    threadutils::thread_pool::ParallelFor(0, nf, [&](int i) {
      if (!m_forces[i]->isPrecomputationParallelized())
        m_forces[i]->preCompute(m_x, m_v, m_interpolated_m, dt);
    });
    ...
    threadutils::thread_pool::ParallelFor(0, nf, [&](int i) {
      if (!m_forces[i]->isPrecomputationParallelized())
        m_forces[i]->preCompute(nx, nv, m_interpolated_m, dt);
    });
...
template <int DIM>
void TwoDScene<DIM>::preComputeLocal(const VectorXs& dx, const VectorXs& dv,
                                     const scalar& dt) {
  assert(dx.size() == dv.size());
...
template <int DIM>
void TwoDScene<DIM>::preComputeInterhair(const VectorXs& dx, const VectorXs& dv,
                                         const scalar& dt) {
  assert(dx.size() == dv.size());
...
void TwoDScene<DIM>::postPreprocess(VectorXs& lambda, VectorXs& lambda_v,
  ...
  if (dx.size() == 0) {
    ...
    threadutils::thread_pool::ParallelFor(0, nf, [&](int i) {
      if (!m_internal_forces[i]->isParallelized())
        m_internal_forces[i]->computeIntegrationVars(
            m_x, m_v, m_interpolated_m, lambda, lambda_v, J, Jv, Jxv, tildeK,
            stiffness, damping, Phi, Phiv, dt);
    });
    ...
    threadutils::thread_pool::ParallelFor(0, nf, [&](int i) {
      if (!m_internal_forces[i]->isParallelized())
        m_internal_forces[i]->computeIntegrationVars(
            nx, nv, m_interpolated_m, lambda, lambda_v, J, Jv, Jxv, tildeK,
            stiffness, damping, Phi, Phiv, dt);
    });
...
template <int DIM>
void TwoDScene<DIM>::interhairPostPreprocess(
    ...
    VectorXs& Phi, VectorXs& Phiv, const VectorXs& dx, const VectorXs& dv,
    const scalar& dt) {
...
void TwoDScene<DIM>::localPostPreprocess(VectorXs& lambda, VectorXs& lambda_v,
                                         TripletXs& J, TripletXs& Jv,
                                         TripletXs& Jxv, TripletXs& tildeK,
                                         TripletXs& stiffness,
                                         TripletXs& damping, VectorXs& Phi,
                                         VectorXs& Phiv, const VectorXs& dx,
                                         const VectorXs& dv, const scalar& dt) {
  ...
  if (dx.size() == 0) {
    ...
    threadutils::thread_pool::ParallelFor(0, nf, [&](int i) {
      for (Force* f : m_hair_internal_forces[i]) {
        if (!f->isParallelized())
          f->computeIntegrationVars(m_x, m_v, m_interpolated_m, lambda,
                                    lambda_v, J, Jv, Jxv, tildeK, stiffness,
                                    damping, Phi, Phiv, dt);
      }
    });
    ...
    threadutils::thread_pool::ParallelFor(0, nf, [&](int i) {
      for (Force* f : m_hair_internal_forces[i]) {
        if (!f->isParallelized())
          f->computeIntegrationVars(nx, nv, m_interpolated_m, lambda, lambda_v,
                                    J, Jv, Jxv, tildeK, stiffness, damping, Phi,
                                    Phiv, dt);
      }
    });
...
template <int DIM>
void TwoDScene<DIM>::getAffectedVars(int icol,
                                     std::unordered_set<int>& affected) {
  for (std::vector<Force*>::size_type i = 0; i < m_forces.size(); ++i)
    m_forces[i]->getAffectedVars(icol, affected);
}
...
void TwoDScene<DIM>::updateEdgeRadii() {
  if (m_massSpringSim) {
    ...
    for (int i = 0; i < ne; ++i) {
      auto& e = m_edges[i];
      scalar l =
          (m_x.segment<DIM>(e.first * DIM) - m_x.segment<DIM>(e.second * DIM))
              .norm();
      m_edge_radii(i) = std::max(
          m_edge_rest_radii(i) * 0.5,
          (1.0 - m_edge_poisson_ratio(i) * (l - m_edge_rest_length(i)) /
                     m_edge_rest_length(i)) *
              m_edge_rest_radii(i));
    }
  } else {
    std::cerr << "edge radii computed via strand parameters in "
                 "TwoDScene<3>::computeMassesAndRadiiFromStrands(), exiting."
              << std::endl;
    exit(1);
  }
}
...
template <int DIM>
void TwoDScene<DIM>::updateMassWithLiquid(int idx, const scalar& liquid_mass,
                                          const scalar& liquid_radii) {
  if (m_massSpringSim || isTip(idx)) {
    for (int r = 0; r < DIM; ++r) {
      m_m(getDof(idx) + r) =
          m_rest_m(getDof(idx) + r) + std::max(0.0, liquid_mass);
    }
  } else {
    for (int r = 0; r < 4; ++r) {
      if (r == 3) {
        m_m(getDof(idx) + r) =
            m_rest_m(getDof(idx) + r) +
            std::max(0.0, liquid_mass) *
                (liquid_radii * liquid_radii + m_radii(idx) * m_radii(idx)) *
                0.5;
      } else {
        m_m(getDof(idx) + r) =
            m_rest_m(getDof(idx) + r) + std::max(0.0, liquid_mass);
...
template <int DIM>
void TwoDScene<DIM>::updateNumConstraintsLocal(int& num_constraint_pos_,
                                               int& num_constraint_vel_,
                                               int& num_J_, int& num_Jv_,
                                               int& num_Jxv_,
                                               int& num_tildeK_) {
...
template <int DIM>
void TwoDScene<DIM>::updateNumConstraintsInterHair(
    int& num_constraint_pos_, int& num_constraint_vel_, int& num_J_,
    int& num_Jv_, int& num_Jxv_, int& num_tildeK_, Vector6i& interhair_param,
    Vector6i& interhair_num) {
  interhair_param = m_constraint_idx;

  for (std::vector<Force*>::size_type i = 0; i < m_inter_hair_forces.size();
       ++i) {
...
template <int DIM>
void TwoDScene<DIM>::updateNumConstraints(int& num_constraint_pos_,
                                          int& num_constraint_vel_, int& num_J_,
                                          int& num_Jv_, int& num_Jxv_,
                                          int& num_tildeK_,
...
template <int DIM>
void TwoDScene<DIM>::storeLambda(const VectorXs& lambda,
...
Vector3s TwoDScene<2>::computeReweightedLiquidGridAngularMomentum() const {
  Vector3s v(0, 0, 0);
  v(2) = ((FluidSim2D*)m_fluid_sim)
             ->computeReweightedParticleGridAngularMomentum();
  return v;
}
...
Vectors<2> TwoDScene<2>::computeParticleWeightedCombinedGridMomentum() const {
  return ((FluidSim2D*)m_fluid_sim)
      ->computeParticleWeightedCombinedGridMomentum();
}
...
template <>
Vector3s TwoDScene<2>::computeParticleWeightedCombinedGridAngularMomentum()
    const {
  return Vector3s(0, 0,
                  ((FluidSim2D*)m_fluid_sim)
...
Vector3s TwoDScene<2>::computeHairWeightedCombinedGridAngularMomentum() const {
  return Vector3s(0, 0,
                  ((FluidSim2D*)m_fluid_sim)
                      ->computeHairWeightedCombinedGridAngularMomentum());
}
...
bool TwoDScene<DIM>::confirmNotNeighborEdgesOnSameHair(
  ...
  if (m_edge_to_hair[efirst] != m_edge_to_hair[esecond])
    return true;
  else if (std::abs(efirst - esecond) < 3)
    return false;
  return true;
}

template <int DIM>
void TwoDScene<DIM>::setVertToDoFMap(const std::vector<int>& vert_to_dof,
                                     const VectorXi& dofs_to_vars,
                                     const std::vector<bool>& tipVerts,
                                     const VectorXi& dof_to_vert) {
  if (m_massSpringSim) {
    std::cerr << "setVertToDoFMap not defined for 2D scene, exiting."
              << std::endl;
    exit(1);
  }
...
void TwoDScene<DIM>::computeMassesAndRadiiFromStrands() {
  if (m_massSpringSim) {
    std::cerr << "computeMassesAndRadiiFromStrands not defined for Mass-Spring "
                 "scene, exiting."
              << std::endl;
    exit(1);
  }
  ...
  for (int f = 0; f < m_forces.size(); ++f) {
    ...
    for (int v = 0; v < strand->getNumVertices(); ++v) {
      ...
      if (v < strand->getNumEdges()) {
        ...
        const scalar mass = strand->m_strandParams->m_density * M_PI * r * r *
